...
double s21::Notation::polish(double x)
{
  double buff = 0;
...
s21::Notation::Lexema s21::Notation::find_lexema(char **pointer)
{
  char *buff = *pointer;
  if (*buff >= '0' && *buff <= '9')
  {
...
double s21::Notation::find_number(char **expr)
{
  double sol = 0;
  double sol_dop = -1;
  char *q = *expr;
  ...
  if (*q == 'e') {
    ...
    sol *= pow(10,expon * expon_znak);
  }
  *expr = q;
  return sol;
}

int s21::Notation::get_postfix(char **expression)
{
  expr.clear();
  std::stack<Lexema> st;
  int unmin = 1;
